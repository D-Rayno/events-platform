...
<template>
  ...
  <AuthLayout>
    <motion.div class="w-full max-w-md" :initial="{ opacity: 0, y: 20 }"
      :enter="{ opacity: 1, y: 0, transition: { duration: 400 } }">
      <!-- Header -->
      <header class="text-center mb-8">
        <motion.h1 class="text-3xl font-bold text-sand-12 mb-2" :initial="{ opacity: 0, y: -10 }"
          :enter="{ opacity: 1, y: 0, transition: { duration: 300, delay: 100 } }">
          Bon retour parmi nous
        </motion.h1>
        <motion.p class="text-sand-11" :initial="{ opacity: 0, y: -10 }"
      ...
      <!-- Form Card -->
      <motion.section class="bg-white shadow-xl rounded-2xl p-8 border border-sand-6"
        aria-label="Formulaire de connexion" :initial="{ opacity: 0, scale: 0.95 }"
        :enter="{ opacity: 1, scale: 1, transition: { duration: 400, delay: 200 } }">
        <form @submit.prevent="handleSubmit" class="space-y-6" novalidate>
          <!-- Email -->
          <Input v-model="form.email" type="email" label="Adresse email" placeholder="jean.dupont@example.com"
            :icon="EnvelopeIcon" :error="getError('email')" required autocomplete="email" @blur="handleBlur('email')"
            @input="clearError('email')" />

          <!-- Password -->
          <Input v-model="form.password" type="password" label="Mot de passe" placeholder="••••••••"
            :icon="LockClosedIcon" :error="getError('password')" required autocomplete="current-password"
            @blur="handleBlur('password')" @input="clearError('password')" />

          <!-- Remember Me & Forgot Password -->
          <div class="flex items-center justify-between">
            <motion.label class="flex items-center gap-2 cursor-pointer group" :initial="{ opacity: 0, x: -10 }"
          ...
          <motion.div :initial="{ opacity: 0, y: 10 }"
            ...
            <Button type="submit" variant="primary" size="lg" :loading="form.processing" :disabled="form.processing"
              full-width>
          ...
          <!-- Register Link -->
          <motion.p class="text-center text-sm text-sand-11" :initial="{ opacity: 0 }"
      ...
      <!-- Additional Info -->
      <motion.div class="mt-6 text-center text-xs text-sand-10" :initial="{ opacity: 0 }"
